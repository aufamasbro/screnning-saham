...
if st.sidebar.button("Analisa"):
    ...
    if data.empty:
        ...
        st.subheader("📊 ANALISIS UNTUK {kode_saham}:")
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Lonjakan Volume   : {'✅ Ada Breakout' if vol_hari_ini > (vol_rata_rata * 2.0) else '❌ Normal/Sepi'}")
        c2.metric("akumulasi bandar   : {'✅ Di Atas Bandar Line (Akumulasi)' if move_bandar else '❌ Di Bawah Bandar Line (Distribusi)'}")


        c4.metric(" ANGKA MA -> MA5: {ma5:.2f} | MA20: {ma20:.2f} | MA50: {ma50:.2f}")

        c3.metric("TRIPLE MA CROSSING : {'✅ Uptrend' if is_uptrend else '❌ Downtrend'}")

        st.markdown("---")
        if is_uptrend and vol_hari_ini > (vol_rata_rata * 2.0) and move_bandar:
            st.succes("SINYAL: VERY BUY 7% - 15%")
